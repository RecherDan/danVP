...
Config::Config(SimulationMode mode)
{
   ...
   m_VTAGESize = (Sim()->getCfg()->getString("VP/VTAGESize").compare("K8") == 0 ) ? K8 : ((Sim()->getCfg()->getString("VP/VTAGESize").compare("K32") == 0 ) ? K32 : UNLIMITED);
   m_VPSIMSize = (Sim()->getCfg()->getString("VP/VPSIMSize").compare("K8") == 0 ) ? K8 : ((Sim()->getCfg()->getString("VP/VPSIMSize").compare("K32") == 0 ) ? K32 : UNLIMITED);
