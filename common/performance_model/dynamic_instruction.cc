...
SubsecondTime DynamicInstruction::getVPCost(Core *core, bool *p_is_mispredict, bool *is_GoodPredicted, int *penalty)
{
   ...
   *penalty=0;
   ...
   if ( uopcache->isUopCacheValid() ) {
	   std::tie(uop_good_prediction, uop_is_mispredict) = ( uopcache->getVPprediction(eip , instruction->getbbhead(), vpinfo.value) );
   ...
   *p_is_mispredict =  (is_mispredict || uop_is_mispredict);
